...
public class DirectionProvider {
	...
	static class Listener implements SensorEventListener, StartableHandlerThread.Callback {
		...
		public void onAccuracyChanged(final Sensor sensor, final int accuracy) {
			/*
			 * There is a bug in Android, which apparently causes this method to
			 * be called every time the sensor _value_ changed, even if the
			 * _accuracy_ did not change. Do not have any code in here.
			 * 
			 * See for example
			 * https://code.google.com/p/android/issues/detail?id=14792
			 */
		}
		...
		/**
		 * Assume that there is an orientation sensor, unless we have really
		 * checked that
		 */
	...
	/**
	 * Take the phone rotation (through a given activity) in account and adjust
	 * the direction.
	 * 
	 * @param direction
	 *            the unadjusted direction in degrees, in the [0, 360[ range
	 * @return the adjusted direction in degrees, in the [0, 360[ range
	 */

	public static float getDirectionNow(final float direction, WindowManager wm) {
		return AngleUtils.normalize(direction + getRotationOffset(wm));
	}

	static float reverseDirectionNow(final float direction, WindowManager wm) {
		return AngleUtils.normalize(direction - getRotationOffset(wm));
	}

	private static int getRotationOffset(WindowManager wm) {
